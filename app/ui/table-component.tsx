...
const TableComponent = <Type extends { id?: number, status?: string, ticket_id?: number }>({ pageTitle, columns, dataArray, dataCountArr }: { pageTitle: string, columns: Array<string>, dataArray: Type[], dataCountArr?: CountDataType }) => {
    ...
    const rowsPerPageArr: Array<number> = [1, 10, 25, 50, 100]
    const tabArray: GridTabType[] = [
        ...
        },
        {
            tabName: 'Approved', modules: [
                {
                    moduleName: 'Leaves',
                    columns: ['Id', 'Emp Id', 'Name', 'Approved By', 'Leave Type', 'Leave Date', 'Days Info', 'Leave Applied', 'Leave Status', 'Actions'],
                    priority: 3
                },
                {
                    moduleName: 'WFH',
        ...
        .filter(tab => tab.modules.find(module => module.moduleName === pageTitle))
        .map(tabName => { return { ...tabName, modules: tabName.modules?.filter(module => module.moduleName === pageTitle) } })
        .sort((a, b) => a.modules[0].priority - b.modules[0].priority)

    const filteredDataArr = dataArray.filter((ele) => currentTab !== 'all' ? ele?.status === currentTab : ele)

    const COLUMN_COUNT = tabArray.find(tab => tab.tabName.toLowerCase().replaceAll(' ', '_') === currentTab)?.modules[0].columns.length

    const handleDropdown = () => setDropdownOpen(prevState => !prevState)

    const handleSelection = ({ tabName }: { tabName: string }) => setCurrentTab(tabName)
    ...
    return (
        <div className="table flex flex-col mt-5 bg-white shadow-sm w-full h-full rounded-xl text-black sticky z-10">
            <div className={clsx("tab-header flex justify-start items-center", {
                ...
                {tabArray.map((tabEle) => {
                    const tabNameId = tabEle.tabName.toLowerCase().replaceAll(' ', '_')
                    return (
                        <div key={tabEle.tabName} id={tabNameId + '-tab-header'}
                            className={clsx("flex flex-row items-center outline-none p-1 mt-2 ml-2 mr-2 mb-0 cursor-pointer",
                                currentTab === tabNameId && 'border-green-color border-b-2',
                                currentTab !== tabNameId && 'border-b-0'
                            )}
                            onClick={() => { handleSelection({ tabName: tabNameId }) }}>
                            <span>
                        ...
                        <tr>
                            {
                                tabArray.find(tabEle => tabEle.modules[0].moduleName !== 'Appraisals') ?
                                    tabArray.filter(tabFilter => tabFilter.tabName.toLowerCase().replaceAll(' ', '_') === currentTab)
                                        .map((tabEle) => (
                    ...
                    <tbody>
                        {
                            filteredDataArr.length > 0 ?
                                ...
                                <tr className="text-center">
                                    <td colSpan={COLUMN_COUNT} className="py-2 text-sm text-box-grey-border h-[30rem]">
                                        <span className="">No {pageTitle} Found</span>
                                    </td>
                                </tr>
                        }
                    </tbody>
                ...
                filteredDataArr.length > 0 ?
                    <div className="tab-footer flex justify-between items-center m-4 min-w-screen">
                            ...
                            <div className="flex flew-wrap flex-col">
                                <button className="flex flex-wrap gap-3 border-[1px] outline-none border-black p-2 rounded-lg" onClick={handleDropdown}>
                                    ...
                                    <svg className={clsx("w-5 h-5", {
                                        'rotate-270': dropDownOpen === false,
                                        'rotate-180': dropDownOpen === true
                                    })} focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="ArrowDropDownIcon"><path d="M7 10l5 5 5-5z"></path></svg>
                                </button>
                                <ul title="rowPerPage" className={clsx("flex flex-wrap flex-col mt-11 w-16 shadow-sm rounded-lg z-20 absolute", {
                                    'hidden': dropDownOpen === false,
                                    'block': dropDownOpen === true
                                })}>
                                    {
                        ...
                        <div className="pageNo flex justify-evenly w-1/6">
                            ...
                            {
                                filteredDataArr.map((arr, index) => (
                                    <button key={index} type="button">
                                        <span className="rounded-lg bg-green-color pl-2.5 pr-2.5 pt-2 pb-2">{index + 1}</span>
                                    </button>
                                ))
                            }
